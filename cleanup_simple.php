<?php
/**
 * Simple Policy Version Cleanup Script for Hostinger
 * Run this by visiting: https://v2insurance.softpromis.com/cleanup_simple.php
 */
...
try {
    ...
    echo "<h2>🔍 Analyzing Policy Versions...</h2>";
    ...
    echo "<p>• Total policies in system: <strong>{$totalPolicies}</strong></p>";
    echo "<p>• Policies with multiple versions: <strong>{$policiesWithDuplicates}</strong></p>";
    echo "<p>• Total versions to delete: <strong>{$totalVersionsToDelete}</strong></p>";
    echo "<p>• Versions to keep: <strong>{$policiesWithDuplicates}</strong></p>";
    echo "</div>";
    ...
    if ($policiesWithDuplicates === 0) {
        ...
        echo "<h3>📋 Sample Policies with Duplicate Versions:</h3>";
        echo "<ul>";
        $sampleCount = 0;
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'cleanup') {
            echo "<h2 class='warning'>🗑️ Starting Cleanup Process...</h2>";
            ...
            echo "<p>✅ Versions deleted: <strong>{$deletedCount}</strong></p>";
            echo "<p>✅ Policies processed: <strong>{$processedCount}</strong></p>";
            echo "<p>❌ Errors encountered: <strong>" . count($errors) . "</strong></p>";
            echo "</div>";
            ...
            if ($remainingDuplicates === 0) {
                ...
                echo "<p class='success'>🎉 Your policy system is now clean!</p>";
            } else {
                echo "<p class='warning'>⚠️ Warning: {$remainingDuplicates} policies still have multiple versions.</p>";
            }
            
        } else {
            echo "<h2 class='warning'>⚠️ Ready to Cleanup</h2>";
            ...
            echo "<p><a href='?action=cleanup' class='btn'>🗑️ DELETE DUPLICATE VERSIONS</a></p>";
